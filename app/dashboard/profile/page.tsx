...
function toStr(value: unknown): string {
  ...
  if (typeof value === 'object') {
    ...
    return String(obj.nameLo || obj.name || obj.code || obj.uuid || JSON.stringify(value))
  }
  return String(value)
}
...
function resolveProfileImage(value: unknown, uid?: string): string {
  ...
  if (typeof value === 'object') {
    const obj = value as Record<string, unknown>
    const byUid = uid ? (obj[uid] as Record<string, unknown> | undefined) : undefined
            ...
            <div className="flex items-center gap-2">
              ...
              {f.masked && (
                <button onClick={f.onToggle} className="text-muted-foreground hover:text-foreground transition-colors">
                  {f.revealed ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                </button>
              )}
            </div>
          </div>
...
export default function ProfilePage() {
  ...
  const profileUser: Employee | null = user
    ? {
        ...
        department: employeeData?.department || employeeData?.workLocation || user.department,
      }
    : null
  ...
  const avatarSrc = uploadedAvatarUrl
  ...
  const handleSubmitUpdate = async () => {
    ...
    try {
      ...
      await submitProfileUpdate({ field: editField, oldValue: currentValue, newValue: editValue })
  ...
  const employmentFields: InfoField[] = [
    ...
    { label: 'ເງິນເດືອນ', value: toStr(NumberFormatter.NoZero(profileUser?.salary || 0)) + ' ກີບ', icon: DollarSign, masked: true, revealed: showSalary, onToggle: () => setShowSalary(v => !v) },
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div className="text-center sm:text-left">
              ...
              <p className="text-muted-foreground">{toStr(profileUser?.jobTitle || profileUser?.position)}</p>
            ...
            {employmentFields.map((f, i) => (
              <div key={i}>
                <div className="flex items-center justify-between py-2">
                  ...
                  {f.editable && (
                    <Button variant="ghost" size="sm" onClick={() => handleEditClick(f.field!, f.value)}>
      ...
      <Card>
        ...
        <CardContent><InfoGrid fields={personalFields} /></CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent><InfoGrid fields={originFields} /></CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent><InfoGrid fields={educationFields} /></CardContent>
      </Card>
